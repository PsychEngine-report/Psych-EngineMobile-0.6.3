...
/**
	...
	Ported Library's Code to Source Code Directly.
	Modified by KralOyuncu.
**/
...
@:final class ASTCTexture extends TextureBase
{
    ...
    @:noCompletion private function new(context:Context3D, data:ByteArray)
    {
        ...
        if (astcLdrExtension == null && astcHdrExtension == null)
        {
            if (!__warned)
            {
                // Replaced CoolUtil with OpenFL's native window alert to prevent breaking Flixel's macro compiler phase
                if (Lib.application != null && Lib.application.window != null) {
                    Lib.application.window.alert("ASTC compression (LDR and HDR) is not available on this device.", "Rendering Error!");
                } else {
                    trace("[ERROR] ASTC compression (LDR and HDR) is not available on this device.");
                }
                __warned = true;
            }
            supported = false;
    ...
    @:noCompletion private function __getImageSize(bytes:ByteArray):Void
    {
        ...
        // Fixed Float -> Int conversion errors
        var blocksX = Std.int(Math.ceil(dimX / blockDimX));
        var blocksY = Std.int(Math.ceil(dimY / blockDimY));
        var blocksZ = Std.int(Math.ceil(dimZ / blockDimZ));
